...
// Route to get a single poll based on its _id
userRoutes.get('/user/getSinglePoll/:pollId', async (req, res) => {
    try {
        const cookieValue = req.query.cookieValue;
        const result = await verifyToken(cookieValue);
        ...
        if (result.success) {
            ...
            const selectedOptions = poll.polls[0].votedUsers
                .filter((votedUser) => votedUser.votedUser === result.user.username)
                .map((votedUser) => votedUser.option);

            const optionsWithSelection = poll.polls[0].options.map((option, index) => ({
                optionText: option.optionText,
                votes: option.votes,
                selected: selectedOptions.includes(index),
            }));
            ...
            const pollDetails = {
                _id: poll.polls[0]._id,
                createdOn: poll.polls[0].createdOn,
                creator: poll.polls[0].creator,
                question: poll.polls[0].question,
                options: optionsWithSelection,
                hasVoted,
                votedUserCount: poll.polls[0].votedUserCount,
            };
